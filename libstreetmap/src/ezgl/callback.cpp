  ...
  if(event->type == GDK_MOTION_NOTIFY) {
    ...
    if(g_mouse_pan.panning_mouse_button_pressed) {
      ...
      g_mouse_pan.prev_y = motion_event->y;
        ...
        if (canvas -> get_camera().get_world().top() > (xy_bounds[2] + 500)) {
          if (dy < 0) dy = 0;
        }
        if (canvas -> get_camera().get_world().bottom() < (xy_bounds[3] - 500)) {
          if (dy > 0) dy = 0;
        }

        if (canvas -> get_camera().get_world().left() < (xy_bounds[1] - 2000)) {
          if (dx > 0) dx = 0;
        }
      if (canvas -> get_camera().get_world().right() > (xy_bounds[0] + 2000)) {
        if (dx < 0) dx = 0;
      }
...
gboolean scroll_mouse(GtkWidget *, GdkEvent *event, gpointer data)
{
  ...
  if(event->type == GDK_SCROLL) {
    ...
    if(scroll_event->direction == GDK_SCROLL_UP) {
      ...
      double map_area = canvas -> get_camera().get_initial_world().area();

      auto g = application -> get_renderer();
      ezgl::rectangle visible_world = g -> get_visible_world();   

      double visible_area = visible_world.area();
      if (visible_area < 0.00001 * map_area) {
          return FALSE;
      }
...
gboolean press_up(GtkWidget *, gpointer data)
{
  ...
  auto canvas = application->get_canvas(main_canvas_id);
  if (canvas -> get_camera().get_world().top() > (xy_bounds[2] + 500)) {
    application -> create_popup_message("Error", "Top bound limit\nUnable to move up");
    return FALSE;
  }
